...
module.exports = (() => {
  ...
  // add revealer to tags
  let revFind = ['<blockquote'];
  function revealer(files, metalsmith, done) {
    ...
    for (let f in files) {
      ...
      for (let r = 0; r < revFind.length; r++) {
        content = content.replace(new RegExp(revFind[r], 'g'), `${revFind[r]} data-revealer="right"`);
      }
