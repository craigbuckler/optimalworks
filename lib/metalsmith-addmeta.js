...
module.exports = (opt) => {
  ...
  return (files, metalsmith, done) => {
    ...
    meta.thumbheight = meta.thumbheight || 12;
    ...
    for (let f in files) {
      ...
      // URL
      file.url = f.replace(/\\/g, '/').replace('index.html', '');

      // default priority
      file.priority = file.priority || 0.5;
      ...
      file.dateSimple =
        ...
        String(file.date.getUTCMonth() + 1).padStart(2, '0') + '-' +
        String(file.date.getUTCDate()).padStart(2, '0');
      ...
      file.thumbheight = file.thumbheight || meta.thumbheight;
    ...
    function addNav(nav) {
      ...
      if (!nav || !nav.child || !nav.child.length) return;

      for (let c = 0; c < nav.child.length; c++) {

        if (nav.child[c].index && files[nav.child[c].index]) files[nav.child[c].index].navrel = {
